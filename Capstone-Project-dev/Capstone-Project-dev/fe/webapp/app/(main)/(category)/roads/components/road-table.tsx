...
import { RoadItem, RoadResponse, RoadTableProps } from "@/types";
...
export const RoadTable = ({
  ...
  const [data, setData] = useState<RoadItem[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const mapped = items.map((item: RoadResponse, index: number) => ({
          id: item.roadId,
  ...
  const renderCell = (item: RoadItem, columnKey: string) => {
    switch (columnKey) {
      case "stt":
        return (
      ...
      default:
        return (
          <span className="text-gray-600 dark:text-default-600">
            {item[columnKey as keyof RoadItem]}
          </span>
        );
    }
  };
