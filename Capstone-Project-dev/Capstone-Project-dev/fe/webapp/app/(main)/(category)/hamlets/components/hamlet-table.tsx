...
export const HamletTable = ({
  ...
  const [data, setData] = useState<HamletItem[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const mapped = items.map((item: HamletResponse, index: number) => ({
          id: item.hamletId,
      ...
      default:
        return (
          <span className="text-gray-600 dark:text-default-600">
            {item[columnKey as keyof HamletItem]}
          </span>
        );
    }
  };
  ...
  return (
    <>
      <GenericDataTable
        isLoading={loading}
        title="Danh sách Thôn/làng"
        columns={HAMLET_COLUMN}
        data={data}
        isCollapsible
